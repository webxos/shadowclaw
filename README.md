...
**Shadowclaw** is a lightweight, self-contained AI agent harness written in C. It uses a local LLM (Ollama) to reason and plan, executes tools (file I/O, HTTP, shell, cron, webhooks), and persistently stores all memories conversation, skills, and cron jobs in a custom arena plus a human‑readable soul file. The agent follows a ReWOO‑style plan‑and‑solve pattern with full tool integration.
